...
float4 main(float2 uv : TEXCOORD) : COLOR
{
	...
	float4 perlinVec = float4(perlin, perlin, perlin, 1.00);
	
	// Get the source color
	float4 color = tex2D(implicitInputSampler, uv);
	float r = color.r + (perlinVec * intensity);
	float g = color.g + (perlinVec * intensity);
	float b = color.b + (perlinVec * intensity);
	float a = color.a + (perlinVec * intensity);
