float zoom : register(C0);
float intensity : register(C1);
float timer : register(C2);
float voronoiversion : register(C3);
float animationspeed : register(C4);
...
const float2 cellular(float3 P) {
...
#define jitter 1.0 // smaller jitter gives more regular pattern
	...
	float3 temp = float3(Pi.x, Pi.x, Pi.x) + float3(-1.0, 0.0, 1.0);
	float3 p = permute(temp);
...
float4 main(float2 uv : TEXCOORD) : COLOR
{
		...
		float2 F = cellular(float3(timer * animationspeed,uv.x * zoom, uv.y * zoom));

		//Voronoi regular pattern
   	float n = (F.y - F.x); 
   	n = n * intensity;
   
   	//Voronoi circle pattern
   	float rings = 1.0 - aastep(0.45, F.x ) + aastep(0.55, F.x);
   	rings = rings * intensity;

		//Decide final return value
   	if(voronoiversion <= 1)
    {
    		return color + float4(n, n, n, n);
    }
    else if(voronoiversion <= 2)
    {
    		return color * float4(n, n, n, n);
    }
    else if(voronoiversion <= 3 )
    {
    		return color * float4(rings,rings,rings, rings);
    }
    else if(voronoiversion <= 4 )
    {
    		return color + float4(rings,rings,rings, rings);
    }
    else
